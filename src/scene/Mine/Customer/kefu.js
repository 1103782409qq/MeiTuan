// "react-native": "0.50.1",是12 Oct 2017
// 对应的时间 @huangminlinux huangminlinux released this on 13 Oct 2017
// "aurora-imui-react-native": "^0.5.10",
...
export default class kefu extends Component {
    ...
    onAvatarClick = (message) => {
        Alert.alert('onAvatarClick')
        // AuroraIController.removeMessage(message.msgId)
    }

    onMsgClick = (message) => {
        Alert.alert('onMsgClick')
        console.log(message)
    ...
    onStatusViewClick = (message) => {
        Alert.alert('onStatusViewClick')
        message.status = 'send_succeed'
        AuroraIController.updateMessage(message)
    }

    onBeginDragMessageList = () => {
        Alert.alert('onBeginDragMessageList')
        this.resetMenu()
        AuroraIController.hidenFeatureView(true)
    }

    onTouchMsgList = () => {
        Alert.alert('onTouchMsgList')
        this.resetMenu()
        AuroraIController.hidenFeatureView(true)
    }
    onTapMessageCell = () => {
        Alert.alert('onTapMessageCell')
    }
    onPullToRefresh = () => {
